...
class LeaderboardCog(commands.Cog):
    ...
    async def create_leaderboard_embed(self, user_id: int, category: str):
        if category == "coin":
        ...
        if self.bot.user.avatar:
            embed.set_author(name="Adventure Quest Leaderboard", icon_url=self.bot.user.avatar.url)
        else:
            embed.set_author(name="Adventure Quest Leaderboard")
    ...
    async def leaderboard(
        ...
        embed = await self.create_leaderboard_embed(user_id, category)
        view = LeaderboardView(self, user_id, category)
        await interaction.response.send_message(embed=embed, view=view)
    
    @commands.command(name="leaderboard", aliases=["lb"])
    async def leaderboard_prefix(self, ctx, category: str = "coin"):
        if category not in ["coin", "level", "win"]:
            category = "coin"
        
        user_id = ctx.author.id
        await self.db.get_user(user_id, ctx.author.name)
...
class LeaderboardView(discord.ui.View):
    ...
    async def update_leaderboard(self, interaction: discord.Interaction, category: str):
        embed = await self.cog.create_leaderboard_embed(self.user_id, category)
