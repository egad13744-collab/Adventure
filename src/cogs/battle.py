...
class BattleCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.db = bot.db
        self.battle_cooldowns = {}  # {user_id: last_battle_timestamp}
        self.COOLDOWN = 10  # 10 detik cooldown
    ...
    async def battle(self, interaction: discord.Interaction):
        ...
        # ===== CEK COOLDOWN =====
        last_battle = self.battle_cooldowns.get(user_id, 0)
        remaining = self.COOLDOWN - (now - last_battle)
        if remaining > 0:
            embed = discord.Embed(
                title="⏳ Cooldown!",
                description=f"Kamu harus menunggu **{int(remaining)} detik** sebelum battle lagi.",
        ...
        # update last battle
        self.battle_cooldowns[user_id] = now
        ...
        embed.add_field(name="📜 Log Pertarungan", value="\n".join(battle_log[-8:]) if battle_log else "Tidak ada aksi", inline=False)
        ...
        if won:
            ...
            embed.add_field(
                name="🎉 MENANG!",
                value=f"Kamu mengalahkan {monster['emoji']} {monster['name']}!\n\n💰 +{coins} koin\n✨ +{base_exp // 2} EXP (player)\n🐾 +{exp_per_animal} EXP (per hewan)",
                inline=False
            )

            if leveled_animals:
                embed.add_field(name="🎊 Level Up Hewan!", value="\n".join(leveled_animals), inline=False)
            if level_result['leveled_up']:
                embed.add_field(name="🎊 LEVEL UP Player!", value=f"Kamu mencapai **Level {level_result['new_level']}**!", inline=False)
        else:
            await self.db.update_battle_stats(user_id, losses=1)
            embed.add_field(
                name="💀 KALAH",
                value=f"{monster['emoji']} {monster['name']} terlalu kuat!\nSemua hewanmu telah jatuh.\n\nGunakan `/animal heal` untuk menyembuhkan tim.",
                inline=False
            )
        ...
        for a in team:
            ...
            team_status.append(f"{emoji} {a['nickname']}: {hp}/{max_hp} HP")

        embed.add_field(name="📊 Status Tim", value="\n".join(team_status), inline=False)
